...
#ifdef NVML_UTF8_API
#pragma message("NVML_UTF8_API macro is obsolete, please
					use PMDK_UTF8_API instead.")
...
namespace pmem {
	...
	namespace detail {
		...
		template < typename T >
		/* XXX "t" below should be probably removed. */
		/* It was only added because cstyle was complaining */
		struct get_type < T * > {
			using type = T * t;
		};
