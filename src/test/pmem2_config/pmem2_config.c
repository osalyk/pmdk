		...
		char *argv[])
{
	...
	int ret = pmem2_config_set_protection(&cfg, PMEM2_PROT_WRITE);
	UT_PMEM2_EXPECT_RETURN(ret, PMEM2_E_INVALID_PROT_FLAG);
	UT_ASSERTeq(cfg.protection_flag, PMEM2_PROT_FROM_FD);

	ret = pmem2_config_set_protection(&cfg,
			PMEM2_PROT_WRITE | PMEM2_PROT_EXEC);
	UT_PMEM2_EXPECT_RETURN(ret, PMEM2_E_INVALID_PROT_FLAG);
	UT_ASSERTeq(cfg.protection_flag, PMEM2_PROT_FROM_FD);
